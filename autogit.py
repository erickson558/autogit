# -*- coding: utf-8 -*-
"""
Git Helper GUI – v0.2.8
- FIX: Configura Git para usar GitHub CLI como credential helper
- FIX: Elimina completamente la autenticación por token en URL
- FIX: Configuración correcta de credential.helper
- FIX: Comando gh repo sync corregido
"""
...
def safe_read_json(path, default):
    try:
        ...
        log_line(f"ERROR leyendo JSON: {e}")
        return default
...
def file_hash(path):
    try:
        ...
        log_line(f"ERROR hash código: {e}")
        return ""
...
class App(tk.Tk):
    def __init__(self):
        ...
        if self.cfg.get("shortcuts_enabled", True):
        ...
        if self.cfg.get("autostart", True):
            self.after(300, self._start_pipeline)
    ...
    def _build_widgets(self):
        ...
        e_secs.pack(side="left")
        e_secs.bind("<FocusOut>", lambda e: self._on_int_change("autoclose_seconds", self.autoclose_secs_var.get()))
        e_secs.bind("<Return>", lambda e: self._on_int_change("autoclose_seconds", self.autoclose_secs_var.get()))

        rowB = ttk.Frame(body)
        rowB.pack(fill="x", pady=(8, 4))
        ...
        ttk.Button(rowB, text="Examinar…",
                   ...
                   state="disabled" if self.cfg.get("follow_exe_folder", True) else "normal").pack(side="left",
                                                                                                   padx=(6, 0))

        rowC = ttk.Frame(body)
        rowC.pack(fill="x", pady=(4, 4))
        ...
        rowE = ttk.LabelFrame(body, text="Autenticación GitHub (OBLIGATORIO)")
        rowE.pack(fill="x", pady=(10, 6))
        ...
        pat_frame = ttk.Frame(rowE)
        pat_frame.grid(row=1, column=0, columnspan=5, sticky="we", padx=6, pady=6)
        ...
        ttk.Button(pat_frame, text="Instrucciones", width=10, command=self._show_pat_instructions).pack(side="left",
                                                                                                       padx=6)
        ...
        logf = ttk.Frame(body)
        logf.pack(fill="both", expand=True, pady=(8, 8))
        self.txt_log = tk.Text(logf, height=14, bg="#0F1620", fg="#C9D1D9", insertbackground="#C9D1D9", relief="flat",
                               wrap="word")
...
📥 INSTALAR Y CONFIGURAR GITHUB CLI (OBLIGATORIO):
1. Descarga desde: https://cli.github.com/
2. Instala GitHub CLI
3. Ejecuta en terminal: gh auth login
4. Selecciona: GitHub.com → HTTPS → Y → Pegar token
5. Verifica con: gh auth status
...
⚠️ IMPORTANTE: GitHub ya no acepta tokens en URLs, usa GitHub CLI."""
    ...
    def _setup_credentials(self, project_path, method):
        """Configuración CORRECTA para usar GitHub CLI como credential helper"""
        if method == "gh" or method == "https_pat":
            pat_token = self.pat_token_var.get().strip()
            github_user = self.github_user_var.get().strip()
            ...
            # 2. Configurar Git para usar GitHub CLI como credential helper
            # ESTA ES LA CONFIGURACIÓN CRÍTICA QUE FALTABA
            config_steps = [
                (["git", "config", "credential.helper", "gh"], "configurar credential helper para GitHub CLI"),
                (["git", "config", "--unset-all", "http.https://github.com.extraheader"], "limpiar headers antiguos"),
            ]
            ...
            # 3. Configurar origin con URL HTTPS estándar (sin token)
            origin_url = f"https://github.com/{github_user}/{repo_name}.git"
            self._ensure_origin(project_path, origin_url)

            # 4. Verificar la configuración
            self.worker_queue.put(("log", "✅ Configurado para usar GitHub CLI como credential helper"))
            return True
    ...
    def _gh_login_with_token(self, token):
        ...
        try:
            ...
            # Método principal: usar --with-token
            try:
                p = subprocess.Popen(
    ...
    def _git_push_with_retries(self, project_path, origin="origin", branch="main"):
        ...
        for i in range(1, attempts + 1):
            ...
            # Comando push estándar - GitHub CLI manejará la autenticación
            ...
            if "authentication failed" in error_text:
                ...
                self.worker_queue.put(("log", "💡 GitHub CLI no está configurado como credential helper"))
                self.worker_queue.put(("log", "💡 Ejecuta: git config --global credential.helper gh"))
                break
    ...
    def _create_remote(self, owner_repo, project_path):
        ...
        rc = self._run_cmd(["gh", "repo", "create", owner_repo, "--public", "--yes"], cwd=project_path)
    ...
    def _remote_url(self, path):
        try:
            return self._run_check_output(["git", "remote", "get-url", "origin"], cwd=path).strip()
        except:
            return ""

    # ... (mantener el resto de los métodos igual que en la versión anterior)

    # Los siguientes métodos se mantienen igual que en tu código original
    # Solo incluyo los signatures para completitud
    ...
    def _on_bool_change(self, key, value):
        ...
        safe_write_json(CONFIG_PATH, self.cfg)
        self._bump_on_config_change(f"{key}={value}")
    ...
    def _browse_folder(self):
        if self.cfg.get("follow_exe_folder", True):
            ...
            return
        path = filedialog.askdirectory(initialdir=self.project_path_var.get() or app_dir(),
    ...
    def _autodetect_repo_name(self):
        ...
        if repo:
            self.repo_name_var.set(repo)
            self._on_str_change("repo_name", repo)
            self._status(f"Repo autodetectado: {repo}")

    def _bump_on_config_change(self, reason=""):
        old = self.cfg.get("version", INITIAL_VERSION)
        new = bump_version(old)
        self.cfg["version"] = new
        safe_write_json(CONFIG_PATH, self.cfg)
        try:
    ...
    def _untrack_list(self, project_path, relpaths):
        ...
        for rel in relpaths:
            ...
            if rc == 0:
                removed.append(rel)
        if removed:
            self.worker_queue.put(("log", "Untrack: " + ", ".join(removed)))
        return removed

    def _bytes_limit_from_cfg(self):
        try:
            return int(self.cfg.get("max_file_size_mb", 95)) * 1024 * 1024
        except:
            return 95 * 1024 * 1024

    def _scan_large_files(self, project_path):
        limit = self._bytes_limit_from_cfg()
        big = []
        for root, dirs, files in os.walk(project_path):
            ...
            for name in files:
                ...
                try:
                    ...
                    pass
        special = "autogit.exe"
        sp = os.path.join(project_path, special)
    ...
    def _git_toplevel(self, path):
        try:
            return self._run_check_output(["git", "rev-parse", "--show-toplevel"], cwd=path).strip()
        except:
            return ""
        ...
        try:
            ...
            if first_time:
                ...
                if self._run_cmd(["git", "init"], cwd=project_path) != 0:
                    self.worker_queue.put(("log", "ERROR en git init"))
                    self.worker_queue.put(("done", None))
                    return
            ...
            for args, label in identity_steps:
                if self._run_cmd(args, cwd=project_path) != 0:
                    self.worker_queue.put(("log", f"ERROR en paso: {label}"))
                    self.worker_queue.put(("done", None))
                    return
            ...
            if rc != 0:
                if self._remove_index_lock_if_any(project_path):
                    rc = self._run_cmd(["git", "add", "."], cwd=project_path)
                if rc != 0:
                    self.worker_queue.put(("log", "ERROR en git add ."))
                    self.worker_queue.put(("done", None))
                    return
            ...
            if first_time or rc_diff_cached != 0:
                ...
                if self._run_cmd(args, cwd=project_path) != 0:
                    self.worker_queue.put(("log", "ERROR en commit."))
                    self.worker_queue.put(("done", None))
                    return
            else:
            ...
            if rc != 0:
                self.worker_queue.put(("log", "❌ ERROR en push. Revisa credenciales y conexión."))
                self.worker_queue.put(("done", None))
                return
    ...
    def _start_pipeline(self):
        if self.running:
            self._status("Ya hay un proceso en ejecución.")
            return
        proj = self.project_path_var.get().strip()
        repo = self.repo_name_var.get().strip()
        if not proj or not os.path.isdir(proj):
            self._status("Ruta del proyecto inválida.")
            self._append_log("ERROR: ruta de proyecto inválida.")
            return
        if not repo:
            self._autodetect_repo_name()
            repo = self.repo_name_var.get().strip()
            if not repo:
                self._status("No se pudo determinar el nombre del repo.")
                return
    ...
    def _stop_pipeline(self):
        if not self.running:
            self._status("No hay proceso en ejecución.")
            return
        self.running = False
        self._append_log("Solicitud de detener recibida… (termina el paso actual)")
    ...
    def on_close(self):
        try:
            ...
            pass
        log_line("Aplicación cerrada por el usuario.")
        self.destroy()
...
if __name__ == "__main__":
    try:
        ...
        log_line("=== Lanzamiento de la aplicación (v0.2.8 - GitHub CLI Fix Completo) ===")
    except Exception as e:
